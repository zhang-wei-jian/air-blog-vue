    ...
    describe('attr', function () {
        describe('#styleToObject', function () {
            it('should parse style string to object', function () {
                ...
                expect(attr_1.styleToObject('fill=red; stroke=blue')).toEqual(ret);
                expect(attr_1.styleToObject('fill=red; stroke=blue;')).toEqual(ret);
            });
            it('should parse key-only string', function () {
                expect(attr_1.styleToObject(';fill=red;;stroke')).toEqual({
                    fill: 'red',
                    stroke: '',
                });
            });
            it('should ingore empty section', function () {
                expect(attr_1.styleToObject(';fill=red;;')).toEqual({ fill: 'red' });
            });
            it('should parse empty string to empty object', function () {
                expect(attr_1.styleToObject('')).toEqual({});
            });
        });
        describe('#mergeAttrs', function () {
            it('shoule merge attrs by extend', function () {
                expect(attr_1.mergeAttrs({ x: 5, y: 10 }, { y: 20 })).toEqual({ x: 5, y: 20 });
            });
            it('should append class attribute', function () {
                expect(attr_1.mergeAttrs({ x: 5, y: 10, class: 'foo' }, { y: 20, class: 'bar' })).toEqual({ x: 5, y: 20, class: 'foo bar' });
            });
            it('shoule merge style object', function () {
                var result = { x: 5, y: 20, style: { fill: 'red', stroke: 'orange' } };
                expect(attr_1.mergeAttrs({ x: 5, y: 10, style: { fill: 'red', stroke: 'blue' } }, { y: 20, style: { stroke: 'orange' } })).toEqual(result);
                expect(attr_1.mergeAttrs({ x: 5, y: 10, style: 'fill=red; stroke=blue' }, { y: 20, style: { stroke: 'orange' } })).toEqual(result);
                expect(attr_1.mergeAttrs({ x: 5, y: 10, style: 'fill=red; stroke=blue' }, { y: 20, style: 'stroke=orange' })).toEqual(result);
                expect(attr_1.mergeAttrs({ x: 5, y: 10, style: { fill: 'red', stroke: 'blue' } }, { y: 20, style: 'stroke=orange' })).toEqual(result);
            });
